...
describe('forward kinematics — initial guess', () => {
  ...
  it('drives the crank pin exactly on its 50 mm circle', () => {
    for (let i = 0; i < 36; i++) {
      ...
      if (r.ok) expect(dist(r.joints.A, geo.O2)).toBeCloseTo(CONFIG.crankLength, 9);
    }
  });
  ...
  it('is consistent at theta = 0 and theta = 360 degrees', () => {
    ...
    if (a.ok && b.ok) {
      ...
      for (const k of ['A', 'B', 'C', 'D', 'E', 'F', 'G'] as const) {
...
describe('branch continuity', () => {
  ...
  it('seeding from the previous frame keeps the solution on one branch', () => {
    ...
    let prev = null as null | { B: { x: number; y: number }; E: { x: number; y: number }; F: { x: number; y: number } };
    let maxJump = 0;
    for (let i = 0; i < 720; i++) {
      const r = solvePose(geo, i * step, prev, { computeSigma: false });
      expect(r.ok).toBe(true);
      if (!r.ok) break;
      if (prev) maxJump = Math.max(maxJump, dist(prev.B, r.joints.B), dist(prev.E, r.joints.E), dist(prev.F, r.joints.F));
  ...
  it('the explicit selector picks a different root than its opposite', () => {
    ...
    expect(a.ok && b.ok).toBe(true);
    if (a.ok && b.ok) expect(dist(a.joints.B, b.joints.B)).toBeGreaterThan(1);
  });
});
  ...
  it('potential energy rises when the assembly is lifted (+y is up)', () => {
    ...
    const lifted = {
      ...
      joints: Object.fromEntries(
        Object.entries(pose.joints).map(([k, v]) => [k, { x: v.x, y: v.y + 100 }]),
      ) as typeof pose.joints,
