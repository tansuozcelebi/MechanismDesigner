...
import { circleCircleIntersection, circleCircleIntersectionEx, dist, interiorAngle, localPoint, smallestSingularValue, thirdSide, unwrapAngle, wrapPi, wrapTwoPi, } from '../src/utils/math';
...
describe('unit conversions', () => {
    ...
    it('deg <-> rad round trip', () => {
        for (const d of [0, 30, 90, 180, -145.3])
...
describe('circle-circle intersection', () => {
    ...
    it('returns null with a positive gap when the circles are too far apart', () => {
        ...
        expect(res.gap).toBeCloseTo(80, 9);
    });
    it('returns null with a positive gap when one circle is inside the other', () => {
        ...
        expect(res.points).toBeNull();
        expect(res.gap).toBeCloseTo(85, 9);
    });
    it('reports near-zero slack at tangency (the dyad dead point)', () => {
        ...
        expect(res.slack).toBeLessThan(1e-9);
    });
...
describe('angle handling', () => {
    ...
    it('unwrapAngle accumulates revolutions', () => {
        let t = 0;
        for (let i = 1; i <= 40; i++)
            t = unwrapAngle(t, wrapPi((i * Math.PI) / 4));
...
describe('segment distance', () => {
    it('crossing segments have zero distance', () => {
        expect(segmentSegmentDistance({ x: -1, y: 0 }, { x: 1, y: 0 }, { x: 0, y: -1 }, { x: 0, y: 1 })).toBeCloseTo(0, 12);
    });
    it('parallel segments are separated by their offset', () => {
        expect(segmentSegmentDistance({ x: 0, y: 0 }, { x: 10, y: 0 }, { x: 0, y: 7 }, { x: 10, y: 7 })).toBeCloseTo(7, 12);
    });
    it('endpoint-to-endpoint case', () => {
        expect(segmentSegmentDistance({ x: 0, y: 0 }, { x: 1, y: 0 }, { x: 4, y: 0 }, { x: 9, y: 0 })).toBeCloseTo(3, 12);
    });
    it('degenerate (point) segments fall back to point distance', () => {
        expect(segmentSegmentDistance({ x: 0, y: 0 }, { x: 0, y: 0 }, { x: 3, y: 4 }, { x: 3, y: 4 })).toBeCloseTo(5, 12);
    });
