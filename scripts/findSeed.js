...
if (widths.length) {
    widths.sort((a, b) => a - b);
    console.log(`  LED path max extent (mm): min ${widths[0].toFixed(1)}, median ${widths[Math.floor(widths.length / 2)].toFixed(1)}, max ${widths[widths.length - 1].toFixed(1)}`);
}
if (bestX) {
