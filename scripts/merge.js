...
for (const f of inputs) {
    ...
    for (const s of data.solutions ?? [])
        all.push({ designArray: s.designArray, score: s.score });
}
all.sort((a, b) => a.score - b.score);
const distinct = [];
for (const c of all) {
    const dup = distinct.some((d) => Math.sqrt(d.designArray.reduce((s, v, i) => s + (v - c.designArray[i]) ** 2, 0)) < 8);
    if (!dup)
        distinct.push(c);
...
writeFileSync(outPath, JSON.stringify({
    ...
    solver: {
        method: 'Differential Evolution (rand/1/bin + current-to-best/1/bin) with constructive feasible seeding and Lamarckian rescue repair, followed by bounded Nelder-Mead refinement',
        mergedFrom: sources,
