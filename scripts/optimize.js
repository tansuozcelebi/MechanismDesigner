...
console.log(`runs=${runs} seed=${baseSeed} pop=${population} gen=${generations} local=${localIterations}`);
const pool = [];
const t0 = Date.now();
for (let r = 0; r < runs; r++) {
    ...
    const res = optimize({
        ...
        onProgress: (p) => {
            const line = `[run ${r + 1}/${runs}] ${p.phase} ${p.generation}/${p.totalGenerations} bestJ=${Number.isFinite(p.bestJ) ? p.bestJ.toFixed(4) : 'inf'} feasible=${p.feasibleCount} evals=${p.evaluations}`;
            if (line !== last) {
                console.log(line);
                last = line;
            }
    ...
    pool.push(...res.best);
    console.log(`[run ${r + 1}] finished, ${res.best.length} candidates, ${res.evaluations} evaluations, ${((Date.now() - t0) /
        1000).toFixed(1)}s elapsed`);
}
pool.sort((a, b) => a.J - b.J);
// Deduplicate and keep the requested number of distinct mechanisms.
const distinct = [];
for (const c of pool) {
    const dup = distinct.some((d) => Math.sqrt(d.x.reduce((s, v, i) => s + (v - c.x[i]) ** 2, 0)) < 5);
    if (!dup)
        distinct.push(c);
    if (distinct.length >= CONFIG.optimizer.keepBest)
        break;
}
...
const solutions = distinct.map((c, idx) => {
    ...
    if (idx < 8) {
        console.log(`#${idx + 1}  J=${m.J.toFixed(4)}  RMS=${m.match.chamferRms.toFixed(2)}mm  ` +
