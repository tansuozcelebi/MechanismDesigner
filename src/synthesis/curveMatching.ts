...
export type RigidTransform = { angle: number; tx: number; ty: number };

export const IDENTITY_TRANSFORM: RigidTransform = { angle: 0, tx: 0, ty: 0 };

export function applyTransform(p: Vec2, T: RigidTransform): Vec2 {
  const c = Math.cos(T.angle);
  const s = Math.sin(T.angle);
  return { x: c * p.x - s * p.y + T.tx, y: s * p.x + c * p.y + T.ty };
}

export const transformAll = (pts: Vec2[], T: RigidTransform): Vec2[] =>
  pts.map((p) => applyTransform(p, T));
...
export function resampleClosed(points: Vec2[], n: number): Vec2[] {
  ...
  if (total < 1e-12) return new Array(n).fill(points[0]);
...
type Packed = { x: Float64Array; y: Float64Array; n: number };

function pack(points: Vec2[]): Packed {
 ...
 */
export function bestRigidAlignment(source: Vec2[], dest: Vec2[]): RigidTransform {
  const n = Math.min(source.length, dest.length, CORRESPONDENCE_SAMPLES);
  if (n < 3) return IDENTITY_TRANSFORM;
...
export type MatchErrors = {
  /** Symmetric Chamfer RMS, mm (brief §12). */
  chamferRms: number;
  /** RMS of LED -> target nearest distances, mm. */
  rmsLedToTarget: number;
  /** RMS of target -> LED nearest distances, mm. */
  rmsTargetToLed: number;
  /** Largest single LED -> target deviation, mm. */
  maxError: number;
  /** Parameterised RMS after the best cyclic alignment, mm (Metric A). */
  paramRms: number;
};
...
export function sizeError(ledPts: Vec2[], targetW: number, targetH: number): number {
 ...
 */
export const heartMatchScore = (rms: number, characteristicLength: number): number =>
  100 * Math.exp(-Math.max(0, rms) / characteristicLength);
