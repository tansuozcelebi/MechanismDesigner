...
export type Candidate = { x: number[]; J: number };

export type OptimizerProgress = {
  phase: 'seeding' | 'global' | 'local' | 'done';
  generation: number;
...
export type OptimizerOptions = {
  ...
  keepBest?: number;
  /** Progressive refinement schedule (brief §8). */
  refineAt?: { medium: number; fine: number };
...
export function optimize(options: OptimizerOptions = {}): OptimizerResult {
  ...
  let evaluations = 0;
  const report = (p: Omit<OptimizerProgress, 'evaluations'>) =>
    options.onProgress?.({ ...p, evaluations });
  ...
  const pushArchive = (x: number[], J: number) => {
    if (!Number.isFinite(J) || J >= 1e4) return; // feasible band only
    // Deduplicate on the design vector (1 mm / 1 deg resolution).
    const key = x.map((v) => Math.round(v)).join(',');
    if (archive.some((c) => c.x.map((v) => Math.round(v)).join(',') === key)) return;
    archive.push({ x: x.slice(), J });
    archive.sort((a, b) => a.J - b.J);
    if (archive.length > keepBest * 4) archive.length = keepBest * 4;
  };
  ...
  for (let g = 0; g < GEN; g++) {
    ...
    const nextLevel: EvalLevel =
      g >= refineAt.fine ? 'fine' : g >= refineAt.medium ? 'medium' : 'coarse';
    if (nextLevel !== level) {
      level = nextLevel;
      for (let i = 0; i < NP; i++) {
        evaluations++;
    ...
    for (let i = 0; i < NP; i++) {
      ...
      while (r3 === i || r3 === r1 || r3 === r2) r3 = Math.floor(rng() * NP);
    ...
    if (g % 5 === 0 || g === GEN - 1) {
      report({
        ...
        message: `DE generation ${g + 1}/${GEN} (N=${
          level === 'coarse' ? CONFIG.samplesCoarse : level === 'medium' ? CONFIG.samplesMedium : CONFIG.samplesFine
        })`,
      });
    }
  }
...
export function nelderMead(
  ...
  const clampX = (x: number[]) =>
    x.map((v, i) => Math.min(bounds[i][1], Math.max(bounds[i][0], v)));

  const simplex: number[][] = [clampX(x0)];
  for (let i = 0; i < n; i++) {
  ...
  for (let iter = 0; iter < maxIter; iter++) {
    const order = fv.map((v, i) => [v, i] as [number, number]).sort((a, b) => a[0] - b[0]);
    const best = order[0][1];
    const worst = order[n][1];
    const second = order[n - 1][1];

    // Converged?
    if (Math.abs(fv[worst] - fv[best]) < 1e-9) break;
