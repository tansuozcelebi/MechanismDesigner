...
import { DESIGN_KEYS, type ContinuousKey } from '../mechanism/types';
...
export function dyadWorstMu(r0: number, r1: number, dmin: number, dmax: number): number {
  const muAt = (d: number) => {
...
function chooseDyadLengths(
  ...
  for (let attempt = 0; attempt < 40; attempt++) {
    ...
    if (dyadTransmissionOk(r0, r1, range.min, range.max, muMin)) return [r0, r1];
  }
  return null;
}
 ...
 */
export function isCrankRocker(lAB: number, lO4B: number, muMin = SEED_MU_MIN): boolean {
  ...
  if (s + l >= sum) return false; // Grashof
...
export function trySampleFeasible(rng: () => number): number[] | null {
  ...
  for (let i = 0; i < PROBE; i++) {
    ...
    const A = v2(
      O2.x + CONFIG.crankLength * Math.cos(th),
      O2.y + CONFIG.crankLength * Math.sin(th),
    );
  ...
  const rCO6: Range = { min: Infinity, max: -Infinity };
  for (const C of Cs) {
    const d = dist(C, O6);
  ...
  const rDG: Range = { min: Infinity, max: -Infinity };
  for (let i = 0; i < PROBE; i++) {
  ...
  const values: Record<ContinuousKey, number> = {
    phi6,
    lAB,
    c3r,
    c3a,
 ...
 * cannot be salvaged.
 */
export function repairDesign(x: number[], muMin = 40): number[] | null {
  const idx = (k: ContinuousKey) => DESIGN_KEYS.indexOf(k);
  const out = x.slice();

  const lAB = out[idx('lAB')];
  const lO4B = out[idx('lO4B')];
  if (!isCrankRocker(lAB, lO4B, Math.min(muMin, DYAD1_MU_MIN))) return null;
