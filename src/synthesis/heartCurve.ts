 ...
 */
export function rawHeart(t: number): Vec2 {
  const s = Math.sin(t);
  return {
    x: 16 * s * s * s,
    y:
...
export type HeartCurve = {
  ...
  width: number;
  height: number;
  /** Total arc length, mm — used as the characteristic scale for scoring. */
  perimeter: number;
};
...
export function makeHeart(
  samples: number = CONFIG.targetSamples,
  width: number = CONFIG.targetWidth,
  height: number = CONFIG.targetHeight,
): HeartCurve {
  const raw: Vec2[] = [];
