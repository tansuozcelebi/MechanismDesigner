...
export class NearestQuery {
    ...
    ny;
    buckets;
    constructor(points, targetCell = 12) {
        this.pts = points;
        let minX = Infinity;
        let minY = Infinity;
        let maxX = -Infinity;
        ...
        this.buckets = Array.from({ length: this.nx * this.ny }, () => []);
    ...
    clampX = (i) => (i < 0 ? 0 : i >= this.nx ? this.nx - 1 : i);
    clampY = (i) => (i < 0 ? 0 : i >= this.ny ? this.ny - 1 : i);
    ...
    distance(q) {
        ...
        if (n === 0)
            return Infinity;
        if (n < 3)
            return Math.hypot(q.x - this.pts[0].x, q.y - this.pts[0].y);
        ...
        for (let ring = 0; ring <= maxRing; ring++) {
            ...
            for (let cy = y0; cy <= y1; cy++) {
                ...
                for (let cx = x0; cx <= x1; cx++) {
                    ...
                    if (ring > 0 && !onYEdge && cx !== gx - ring && cx !== gx + ring)
                        continue;
            ...
            if (bestIdx >= 0 && ring * this.cell > Math.sqrt(bestD2))
                break;
        }
        if (bestIdx < 0)
            return Infinity;
