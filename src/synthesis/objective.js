...
import { arrayToDesign, buildGeometry, lengthViolation, ratioPenalty, } from '../mechanism/mechanism';
...
import { bestRigidAlignment, chamferError, invertTransform, sizeError, transformAll, } from './curveMatching';
...
export function emptyMetrics(J, reason) {
    return {
        J,
        valid: false,
        rejectReason: reason,
 ...
 * optimiser minimised.
 */
export function evaluateDesign(x, options = {}) {
    ...
    if (!sw.fullRotation) {
        ...
        return emptyMetrics(BAND_ASSEMBLY * (1 + failFrac) + Math.min(1000, meanGap), `Full rotation FAILED: ${sw.validFrames}/${sw.frames} frames`);
    }
    if (sw.assemblyJumps > 0) {
        return emptyMetrics(BAND_ASSEMBLY * 0.9 + sw.assemblyJumps, `ASSEMBLY MODE JUMP on ${sw.assemblyJumps} frame(s)`);
    }
    return scoreSweep(geo, sw, w, options);
}
/** Score an already-computed sweep. Split out so the UI can reuse a live sweep. */
export function scoreSweep(geo, sw, w = CONFIG.weights, options = {}) {
    ...
    // --- Closure ------------------------------------------------------------
    const eClosure = sw.maxLoopClosureError / CONFIG.loopClosureTol +
    ...
    for (const p of sw.poses) {
        for (const mu of p.transmissionAngles)
            muEff = Math.min(muEff, Math.min(mu, 180 - mu));
    }
    // Grows like 1/sin^2: 60 deg -> 0, 40 deg -> 1.25, 20 deg -> 8, 10 deg -> 35.
    const eSing = muEff < CONFIG.scales.muReference ? (CONFIG.scales.muReference / Math.max(muEff, 0.5)) ** 2 - 1 : 0;
    ...
    if (options.computeGravity !== false) {
        ...
        for (let i = 0; i < sw.poses.length; i += gstride) {
            ...
            if (Number.isFinite(t))
                peakTau = Math.max(peakTau, Math.abs(t));
        }
    }
    const terms = {
    ...
    if (rejected)
        J += BAND_ASSEMBLY * 0.5 + (CONFIG.scales.muHardReject - muEff);
    return {
        J,
        valid: !rejected,
...
export function objectiveValue(x, level = 'coarse') {
