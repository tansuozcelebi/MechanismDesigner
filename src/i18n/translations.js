...
export const en = {
    ...
    'display.hint': 'Drag the orange crank to turn the motor by hand. Drag empty space to pan, wheel to zoom.',
    /* ---- cycle verification ---- */
    ...
    'cycle.effectiveMuTip': "min(μ, 180−μ) over the cycle; this frame's physical ceiling is 44.75°",
    ...
    'live.gravityTorqueTip': 'dU/dθ — quasi-static torque the motor must supply against gravity',
    'live.motorTorque': 'Est. motor torque',
    'live.motorTorqueTip': 'Lagrange estimate: gravity + inertial terms at constant motor speed',
    ...
    'live.interferenceTip': 'Coplanar interference at 12 mm bar width between bodies not sharing a joint',
    'live.clear': 'CLEAR',
    'live.pairs': '{n} pair(s)',
    ...
    'target.scoreNote': 'Display score only — 100·exp(−RMS/{n}). Engineering decisions use the millimetre values above.',
    /* ---- objective ---- */
    ...
    'torque.note': 'τ_g(θ) = dU/dθ by central difference on the analytic solver, U = Σ mᵢ g y_comᵢ.',
    ...
    'links.mass': 'Mass',
    'links.layer': 'Ly',
    'links.note': 'Lengths are fixed rigid-body dimensions; angles update every frame. “Ly” is the assembly layer (parallel plane) the body sits in. All members are constrained to {min}–{max} mm.',
    /* ---- topology ---- */
    'topology.title': 'Topology',
    'topology.mobility': 'Mobility = {n}',
    'topology.loops': '{n} independent loops',
    'topology.counts': '8 links / 10 joints',
    'topology.note': 'Solved as three RRR Assur dyads in series — every joint comes from a closed-form circle–circle intersection, so no Newton iteration is used anywhere.',
    /* ---- design vector ---- */
    'design.title': 'Design Vector — {label}',
    'design.fixedNote': 'Fixed by the brief: O2 = (0, 0), |O2 O4| = {a} mm, |O4 O6| = {b} mm, crank |O2 A| = {c} mm.',
    ...
    'opt.note': 'Differential Evolution (rand/1 + current-to-best/1) with a constructive feasible seed population, then bounded Nelder–Mead refinement. Runs in a Web Worker so the canvas keeps animating. Sampling refines {a} → {b} → {c} frames per revolution.',
...
export const tr = {
    ...
    'display.hint': 'Motoru elle çevirmek için turuncu krankı sürükleyin. Kaydırmak için boş alanı sürükleyin, yakınlaştırmak için tekerleği kullanın.',
    /* ---- çevrim doğrulama ---- */
    ...
    'cycle.effectiveMuTip': 'Çevrim boyunca min(μ, 180−μ); bu şasenin fiziksel tavanı 44,75°',
    ...
    'live.gravityTorqueTip': 'dU/dθ — motorun yerçekimine karşı sağlaması gereken yarı-statik moment',
    'live.motorTorque': 'Tahmini motor momenti',
    'live.motorTorqueTip': 'Lagrange tahmini: sabit motor hızında yerçekimi + atalet terimleri',
    ...
    'live.interferenceTip': 'Ortak mafsalı olmayan gövdeler arasında 12 mm kol genişliğinde eş düzlem girişimi',
    'live.clear': 'TEMİZ',
    'live.pairs': '{n} çift',
    ...
    'target.scoreNote': 'Yalnızca gösterim puanı — 100·exp(−RMS/{n}). Mühendislik kararlarında yukarıdaki milimetre değerleri kullanılır.',
    /* ---- amaç fonksiyonu ---- */
    ...
    'torque.note': 'τ_g(θ) = analitik çözücü üzerinde merkezi farkla dU/dθ, U = Σ mᵢ g y_agmᵢ.',
    ...
    'links.mass': 'Kütle',
    'links.layer': 'Kat',
    'links.note': 'Uzunluklar sabit rijit gövde ölçüleridir; açılar her karede güncellenir. “Kat”, gövdenin bulunduğu montaj katmanıdır (paralel düzlem). Tüm elemanlar {min}–{max} mm aralığında sınırlandırılmıştır.',
    /* ---- topoloji ---- */
    ...
    'topology.note': 'Seri bağlı üç RRR Assur diyadı olarak çözülür — her mafsal kapalı formda çember–çember kesişiminden gelir, bu yüzden hiçbir yerde Newton iterasyonu kullanılmaz.',
    /* ---- tasarım vektörü ---- */
    'design.title': 'Tasarım Vektörü — {label}',
    'design.fixedNote': 'Şartnameyle sabit: O2 = (0, 0), |O2 O4| = {a} mm, |O4 O6| = {b} mm, krank |O2 A| = {c} mm.',
    ...
    'opt.note': 'Yapıcı bir uygulanabilir başlangıç popülasyonu üzerinde Diferansiyel Evrim (rand/1 + current-to-best/1), ardından sınırlı Nelder–Mead iyileştirmesi. Tuval akıcı kalsın diye Web Worker içinde çalışır. Örnekleme tur başına {a} → {b} → {c} kareye incelir.',
